function closeAllOpenedElements() {
  ...
  function openDialog(id) {
    ...
    if (id === 'order_dialog') {
        const dialogContent = document.getElementById('single_offe_order_dialog');
        if (dialogContent) {
            dialogContent.innerHTML = getShowOrderDialogContentTemplate();
        }
    }
}
...
function getPersonImgPath(currentUser) {
    if (!currentUser) {
        return "./assets/icons/profile_pic.svg";
    }

    if (currentUser.profile_image) {
        return formatImagePath(currentUser.profile_image);
    }

    if (currentUser.file) {
        return formatImagePath(currentUser.file);
    }
