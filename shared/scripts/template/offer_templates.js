...
function getOfferDialogTemplate() {
    if (!Array.isArray(currentOffers) || !currentOffer) {
        console.error("Fehler: currentOffers ist keine Liste oder currentOffer ist null:", currentOffers, currentOffer);
    ...
    <div onclick="stopProp(event)" class="m_auto dialog_content large_form d_flex_cc_gl f_d_c m_auto pos_rel">
                    ...
                    <form onsubmit="onOfferSubmit(event)" id="add_offer_form" class="d_flex_cc_gl f_d_c w_full" novalidate>
                        ...
                        <div class="form_group">
                <label for="offer_price">Preis (€)</label>
                <input placeholder="0.00" value="${currentOffer?.price || ''}" 
                       class="input_field" type="number" id="offer_price" name="offer_price" step="0.01" required>
                <p id="offer_price_error" class="form_error d_none">Bitte ausfüllen!</p>
            </div>

            <!-- Lieferzeit -->
            <div class="form_group">
                <label for="offer_delivery_time">Lieferzeit (Tage)</label>
                <input placeholder="0" value="${currentOffer?.delivery_time_in_days || ''}" 
                       class="input_field" type="number" id="offer_delivery_time" name="offer_delivery_time" required>
                <p id="offer_delivery_time_error" class="form_error d_none">Bitte ausfüllen!</p>
            </div>
...
function getOfferDetailDialogTemplateList() {
    ...
    if (!Array.isArray(currentOffer.details)) {
        return '<p>Fehler beim Laden der Angebotsdetails</p>';
    }
    if (currentOffer.details.length === 0) {
        return '<p>Keine Angebotsdetails vorhanden</p>';
    }
    ...
    for (let index = 0; index < currentOffer.details.length; index++) {
        ...
        if (!detail.features) {
            console.warn(`Features fehlen für Detail mit Typ: ${detail.offer_type}`);
            detail.features = []; 
        }
        detailTemplateList += getOfferDetailDialogTemplate(detail);
    }
...
function getOfferDetailDialogTemplate(detail) {
    ...
    const title = detail.title || "Unbenannter Titel";
    const price = detail.price !== null && detail.price !== undefined ? detail.price : "0.00";
    const deliveryTime = detail.delivery_time_in_days !== null && detail.delivery_time_in_days !== undefined
        ? detail.delivery_time_in_days
        : "0";
    const revisions = detail.revisions !== null && detail.revisions !== undefined ? detail.revisions : 0;

    const checked = revisions === -1 ? "checked" : "";
    const revisionsCount = revisions <= 0 ? 0 : revisions;
    const revisionsDisabled = revisions === null || revisions > 0 ? "" : "disabled";
    ...
    <section class="variant_section section_group d_flex_cs_gl f_d_c">
        ...
        <h2>${detail.offer_type || "Unbekannt"}</h2>
        ...
        <div class="order_item_detail w_full w_full d_flex_cc_gl f_d_c">
            <div class="w_full d_flex_cc_gm f_d_c">
                <div class="form_group">
                    ...
                    <input placeholder="Gib hier die Beschreibung ein" value="${title}" onkeydown="hideFormErrors(['offer_title_${detail.offer_type}_error'])" class="input_field" type="text" id="offer_title_${detail.offer_type}"
                ...
                <div class="form_group">
                    ...
                    <input placeholder="0.00" value="${price}" onkeydown="hideFormErrors(['offer_price_${detail.offer_type}_error'])" class="input_field" type="number" id="offer_price_${detail.offer_type}"
                ...
                <div class="form_group">
                    ...
                    <input placeholder="0" value="${deliveryTime}" onkeydown="hideFormErrors(['offer_delivery_time_${detail.offer_type}_error'])" class="input_field" type="number" id="offer_delivery_time_${detail.offer_type}"
