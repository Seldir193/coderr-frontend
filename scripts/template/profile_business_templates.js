function getBusinessProfilePageTemplate(currentUser, currentOrder,  currentOffers, currentReviews) {
    return `
            ...
            <section class="business_section_profile d_flex_cs_gl f_d_c">
              ...
              <button onclick="redirectToProfile(${currentUser.id}, '${currentUser.type || 'business'}')" 
...
function getBusinessOrderTemplateList() {
    currentOrders.sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
    let orderListHTML = ``;
...
function getBusinessOrderTemplate(order) {
    ...
    const customer_user = order.user_details || order.user; 
    const customerName = customer_user
        ? `${customer_user.first_name || ''} ${customer_user.last_name || ''}`.trim() || customer_user.username
        : "Unbekannter Kunde";

    if (!customer_user) {
        showToastMessage(true, [`Kunde mit ID ${order.user} nicht gefunden.`]);
    }
        ...
        <li class="order_item_box d_flex_cc_gm w_full f_d_c">
            ...
            <div class="order_item d_flex_cs_gm">
                ...
                <div open=false closable="true" onclick="toggleOpen(this); stopProp(event)" status="${order.status}"
                    ...
                    <p>${orderStatus[order.status] || 'Unbekannter Status'}</p>
            ...
            <div class="w_full order_item_detail d_flex_cs_gm f_d_c">
                ...
                <p class="link" onclick="${customer_user ? `redirectToProfile(${customer_user.id}, '${customer_user.type || 'unknown'}')` : 'console.warn(\'Kundendaten fehlen\')'}">
                    
                        <strong>Käufer:</strong> ${customerName}
                    </p>
                    <p><strong>Titel:</strong> ${order.offer_title || 'Unbekannt'}</p>
                    <p><strong>Lieferzeit:</strong> ${order.offer_delivery_time || 'Nicht angegeben'} Tage</p>
                    <p><strong>Revisionen:</strong> ${getOrderRevisionTemplate(order.offer_revision_limit)}</p>
                    <p><strong>Preis:</strong> ${parseFloat(order.offer_price).toFixed(2)}€</p>
...
function getBusinessProfileTemplate(currentUser) {
    ...
    return `      
        ...
        <div class="d_flex_cs_gm f_d_c profile_detail">
            ...
            <img class="profile_img"  src="${getPersonImgPath(currentUser) 
            ...
            <p>${currentUser.description || 'Keine Beschreibung verfügbar.'}</p>
        </div>
        <div class="d_flex_cs_gl f_d_c">
            <div class="d_flex_cs_gm f_d_c w_full">
                ...
                <p class="d_flex_cc_gm">
                    ...
                    ${currentUser.email || 'Nicht angegeben'}
                </p>
                <p class="d_flex_cc_gm">
                    <img src="./assets/icons/call.svg" alt="">
                    ${currentUser.tel || 'Nicht angegeben'}
                </p>
                <hr>
                <p class="d_flex_cc_gm">
                    <img src="./assets/icons/location_on.svg" alt="">
                    Standort: ${currentUser.location || 'Nicht angegeben'}
                </p>
                <p class="d_flex_cc_gm">
                    <img src="./assets/icons/person.svg" alt="">
                    Mitglied seit ${formatToMonthYearAndDay(currentUser.created_at || new Date())}
                </p>
                <hr>
                <p class="d_flex_cc_gm">
                    <img src="./assets/icons/schedule.svg" alt="">
                    Verfügbarkeit: ${currentUser.working_hours || 'Nicht angegeben'}
                </p>
            </div>
        </div>
    `;
    ...
    return `<div onclick="stopProp(event)" class="m_auto dialog_content small_form d_flex_cc_gl f_d_c">
                    ...
                    <form onsubmit="businessEditOnsubmit(event)" class="d_flex_cc_gm f_d_c w_full pos_rel">
                        ...
                        <div class="image_input_box">
                            <img id="business_profile_img_input_output" class="profile_img_l" src="${getPersonImgPath(currentUser)}" alt="Aktuelles Profilbild">
