    ...
    return `
        ...
        <div class="d_flex_cs_gxl profile_customer">
                    <div class="d_flex_cc_gm f_d_c">
                       <img class="profile_img_l" src="${getPersonImgPath(currentUser)}" alt="Profilbild">
                    </div>
                    <div class="d_flex_cs_gl f_d_c offer_card">
                        ...
                        <div class="d_flex_cs_gm f_d_c w_full">
                            ...
                            <button onclick="redirectToCustomerProfile(${currentUser.id})" class="std_btn btn_prime d_flex_cc_gm w_full">
    ...
    <div onclick="stopProp(event)" class="m_auto dialog_content small_form d_flex_cc_gl f_d_c">
                    <form onsubmit="customerEditOnsubmit(event)" class="d_flex_cc_gm f_d_c w_full pos_rel">
                        ...
                        <div class="image_input_box">
                            <img id="customer_profile_img_input_output" class="profile_img_l" src="${getPersonImgPath(currentUser)}" alt="Aktuelles Angebotsbild">
...
function getCustomerOrderTemplate(order) {
    if (!order || typeof order !== 'object' || !order.id || !order.created_at || !order.status ) {
        return `
            <li class="order_item_box d_flex_cs_gm w_full f_d_c">
                <p>Fehler: Ungültige Bestelldaten.</p>
            </li>`;
    }
    ...
    return `
        <li class="order_item_box d_flex_cc_gm w_full f_d_c">
            <button open=false class="std_btn btn_prime pad_s order_btn_close d_flex_cc_gm" onclick="toggleOpen(this)">
            ...
            <div class="w_full order_item_detail d_flex_cs_gm f_d_c">
                <div class="order_info d_flex_cs_gm f_d_c">
                    ...
                    <p class="link" onclick="redirectToProfile(${order.business_user?.id || 'undefined'}, '${order.business_user?.type || 'unknown'}')">
                         <strong>Anbieter:</strong> ${order.offer_provider || "Unbekannt"}
                    </p>
                    <p><strong>Titel:</strong> ${order.offer_title || "Kein Titel verfügbar"}</p>
                    <p><strong>Lieferzeit:</strong> ${order.offer_delivery_time ? `${order.offer_delivery_time} Tage` : "Nicht angegeben"}</p>
                
                    <p><strong>Revisionen:</strong> ${order.offer_revision_limit !== undefined ? order.offer_revision_limit : "Nicht verfügbar"}</p>
                    <p><strong>Preis:</strong> ${order.offer_price ? `${parseFloat(order.offer_price).toFixed(2)}€` : "Nicht verfügbar"}</p>
                    <ul class="feature_list">
                    ${getOrderFeatureListTemplate(features)}
                    </ul>
